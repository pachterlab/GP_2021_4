...
figure3b<-DimPlot(atac_brain_scatak, reduction = "umap", label = TRUE, pt.size = 0.5, label.size=5) + NoLegend()
library(cowplot)
plot_grid(figure3a, figure3b, ncol=2)

png("Figure3_AB.png", width=960, height=400)
plot_grid(figure3a, figure3b, ncol=2)
dev.off()
...
png("circos.png", width=600, height=600)
...
plot_density<-ggplot(gwas_atac_new, aes(colour=Cell_type, x=log10(ATAC_signal+1))) + geom_density() + scale_color_manual(values=c("red", "green", "blue", "cyan", "magenta", "yellow")) + ggtitle("chromatin accessibility of AD SNPs")

plot_density
png("gwas_atac_density.png", width=360, height=600)
plot_density
dev.off()
...
plot_feature
png("./umap_hic.png", width = 480, height = 360)
plot_feature
dev.off()
